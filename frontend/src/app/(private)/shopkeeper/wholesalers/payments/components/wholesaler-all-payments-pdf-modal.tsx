...
interface WholesalerAllPaymentsPdfModalProps {
    open: boolean;
...
export function WholesalerAllPaymentsPdfModal({ open, onOpenChange, filters }: WholesalerAllPaymentsPdfModalProps) {
    ...
    const generatePdf = useCallback(async () => {
        ...
        try {
            ...
            doc.text("WHOLESALER PAYMENTS", 14, 22);

            doc.setFontSize(10);
            doc.setFont('helvetica', 'normal');
            doc.text("Comprehensive Payment History Report", 14, 30);

            doc.setFontSize(8);
            doc.text(`File Ref: WPMT-${format(now, 'yyyyMMdd')}`, 196, 18, { align: 'right' });
            ...
            doc.setFont('helvetica', 'normal');
            let filterDesc = "All Wholesaler Payments";
            ...
            if (filters.paymentMethod && filters.paymentMethod !== 'all') {
                filterDesc += ` | Method: ${filters.paymentMethod.toUpperCase()}`;
            }
            if (filters.search) {
                filterDesc += ` | Search: "${filters.search}"`;
            }

            doc.text(filterDesc, 45, 52);

            // --- Table Section ---
            const tableColumn = ["SL", "WHOLESALER", "AMOUNT", "METHOD", "DATE", "NOTES"];
            ...
            const tableRows = payments.map((p: any, index: number) => [
                ...
                format(new Date(p.createdAt), 'dd/MM/yy hh:mm a'),
                p.notes || '-'
            ]);
            ...
            autoTable(doc, {
                ...
                columnStyles: {
                    ...
                    1: { fontStyle: 'bold', cellWidth: 'auto' }, // WHOLESALER
                    2: { halign: 'right', fontStyle: 'bold', textColor: [21, 128, 61], cellWidth: 30 }, // AMOUNT
                    3: { halign: 'center', cellWidth: 25 },    // METHOD
                    4: { halign: 'center', cellWidth: 35 },    // DATE
    ...
    const handleDownload = () => {
        if (pdfDataUrl) {
            ...
            link.download = `Wholesaler_Payments_${format(new Date(), 'dd-MMM-yyyy')}.pdf`;
            link.click();
        }
    };
            ...
            <DialogContent className="w-[95vw] sm:max-w-4xl h-[85vh] sm:h-[90vh] flex flex-col p-0 gap-0 overflow-hidden border-none sm:border">
                <DialogHeader className="px-4 sm:px-6 py-3 sm:py-4 border-b flex flex-row items-center justify-between space-y-0 bg-white z-20">
                    <DialogTitle className="flex items-center gap-2 text-base sm:text-lg truncate mr-2">
                        <Printer className="h-4 w-4 sm:h-5 sm:w-5 text-green-600 shrink-0" />
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Button
                            ...
                            className="bg-green-600 hover:bg-green-700 h-8 sm:h-9 px-3 sm:px-4 text-xs sm:text-sm"
                        >
                ...
                <div className="flex-1 bg-gray-50 p-2 sm:p-4 overflow-hidden relative flex flex-col">
                    {loading && !pdfDataUrl && (
                        <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-sm z-30">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <p className="text-sm font-medium text-gray-600 italic">Preparing Payments Report...</p>
                            </div>
                        </div>
                    )}
                    ...
                    {error ? (
                        ...
                        <PdfViewer url={pdfDataUrl} title="Payments Report Preview" />
                    ) : (
